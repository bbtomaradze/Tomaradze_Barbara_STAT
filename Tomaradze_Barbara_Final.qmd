...
sum <- 0
while (TRUE) {
  integer <- sample(1:10, 1, replace=TRUE)
  count <- count + 1
  print(integer)
  sum <- sum + integer
  if (sum > 29) {
...
sprintf("You generated %d numbers that sum to %d", count, sum)
```
...
bmi_results <- bmi_calculation(72, 200)
sprintf("Your BMI is %s with a value of %s.", bmi_results[2], bmi_results[1])
```
...
```{r}
vector_matrix <- cbind(vec1, vec2, vec3)
print(vector_matrix)
```
...
```{r}
max_col <- apply(vector_matrix, 2, max)
print(max_col)
```
...
```{r}
list_sum <- lapply(vector_list, sum)
list_sum_even <- vector("logical", 3)

i <- 1
for (sum in list_sum) {
  if ((sum %% 2) == 0) {
    list_sum_even[i] <- TRUE
  }
}
print(list_sum_even)
```
...
```{r}
max_weight_index <- which.max(fruit_info$weight)
print(fruit_info[max_weight_index, "fruit"])
```
...
```{r}
# There's an error in my code so I couldn't render to HTML with my code -> it is in my .qmd file though
```
